...
class BoolQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        encoded_review = self.tokenizer.encode_plus(
            ...
            return_tensors="pt", # Yoonjung's notes: "pt" means pytorch tensor
            padding="max_length",
            truncation=True
        )
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, small_subset=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in tqdm(enumerate(train_dataloader)):
            ...
            output = mymodel(input_ids = id, attention_mask=mask, labels = label)
            predictions = output.logits
            ...
            # zero the gradients
            optimizer.zero_grad() # Yoonjung's notes: gradients are reset after updating
            # your code ends here
...
def pre_process(model_name, batch_size, device, small_subset):
    ...
    lora_model = LoraModel(pretrained_model, config, "default")
